...
// Server lists the workflow service endpoint HTTP handlers.
type Server struct {
	Mounts   []*MountPoint
	List     http.Handler
...
// New instantiates HTTP handlers for all the workflow service endpoints using
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"Get", "GET", "/workflows/{workflowNameOrId}"},
		},
...
func (s *Server) Service() string { return "workflow" }
...
// Mount configures the mux to serve the workflow endpoints.
...
// MountListHandler configures the mux to serve the "workflow" service "list"
// endpoint.
...
// the "workflow" service "list" endpoint.
func NewListHandler(
...
// MountRegisterHandler configures the mux to serve the "workflow" service
// "register" endpoint.
...
// calls the "workflow" service "register" endpoint.
...
// MountGetHandler configures the mux to serve the "workflow" service "get"
// endpoint.
func MountGetHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/workflows/{workflowNameOrId}", f)
}

// NewGetHandler creates a HTTP handler which loads the HTTP request and calls
// the "workflow" service "get" endpoint.
func NewGetHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
